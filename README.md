...
- **동시성 제어**: Redis 원자성과 Redisson 분산 락을 활용해 동시성 처리 최적화
---
...
### Kafka를 이용한 주문 - 결제 - 재고반영 속도 개선   --> 속도개선과 패턴적용 분리해서 정리하기
...
### 레디스 캐싱을 이용한 제품 상세조회 성능 개선
- 문제 상황 : 상세페이지 접근시 매번 쿼리문이 날아감, 동시접속자가 많아지면  속도 저하 및 오류
- 해결 방법 : 레디스 캐싱을 통한 속도 개선
- ### **성능 개선 결과** (표만들어서 정리)

1. **TPS (초당 요청 처리량)**:

   **22.4% 개선** (388.05 TPS → 476.15 TPS)

2. **평균 응답 시간**:

   **81.7% 개선** (263.37ms → 48.18ms)

3. **최대 응답 시간**:

   **87.9% 개선** (6.75초 → 814.2ms)

4. **P90 응답 시간**:

   **68.4% 개선** (274.09ms → 86.75ms)

5. **P95 응답 시간**:

   **87.4% 개선** (1.17초 → 148.09ms)

### 모놀리식에서 MSA로 변환 후 ErrorException 처리시 500에러 통합 표시
...
## 🐞 트러블 슈팅
- 모놀리식 ErrorException 적용이 msa에서 500에러 + 해당 에러원인의 모듈에서 콘솔창으로 나타남 ->ErrorDecoder
- Kafka Consumer 처리 속도 지연으로 인해 Producer 메시지 전송 지연 발생  ->  kafkaBatchLister와 sql batchLister를 이용